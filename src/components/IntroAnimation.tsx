  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* 简洁的水彩背景 - 只保留几个柔和的 */}
        <motion.div
          className="absolute -left-20 -top-20 w-[500px] h-[500px] rounded-full"
          style={{
        ...
        />
        <motion.div
          className="absolute right-0 top-1/4 w-[400px] h-[400px] rounded-full"
          style={{
        ...
        />
        <motion.div
          className="absolute left-1/3 bottom-0 w-[450px] h-[450px] rounded-full"
          style={{
        ...
        {phase >= 1 && phase < 3 && (
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            {[...Array(6)].map((_, i) => (
              <motion.path
                key={i}
                d={`M 100% ${25 + i * 10}% Q 60% ${48 + (i - 3) * 2}% 25% 50%`}
                fill="none"
                stroke={colors[i]}
                strokeWidth="1.5"
                strokeOpacity="0.4"
        ...
        {/* 粒子流 - 减少数量 */}
        {phase >= 1 && phase < 3 && [...Array(15)].map((_, i) => (
          <motion.div
            key={i}
            ...
            >
              <div className="relative" style={{ x: "-50%", y: "-50%" }}>
                <Image
                  ...
                  width={280}
                  height={140}
                  className="w-[200px] h-auto md:w-[260px] lg:w-[320px]"
                />
              </div>
            </motion.div>
          );
        ...
        {/* 最终 Logo + 进入按钮 - 左右布局 */}
        <motion.div
          className="absolute z-50 inset-0 flex items-center justify-center gap-8 md:gap-16 lg:gap-24 px-8"
          initial={{ scale: 0, opacity: 0 }}
        ...
        >
          ...
          <Image
            ...
            width={600}
            height={300}
            className="w-[280px] h-auto md:w-[400px] lg:w-[500px] xl:w-[600px]"
            priority
          ...
          >
            {/* 外框 */}
            <div className="relative px-10 py-4 md:px-14 md:py-5 border-2 border-black bg-transparent group-hover:bg-black transition-colors duration-200">
              {/* 装饰角 */}
              ...
              {/* 文字 */}
              <span className="text-base md:text-lg font-medium tracking-[0.3em] text-black group-hover:text-white transition-colors duration-200 flex items-center gap-4">
                ENTER
                <motion.span
        ...
        <motion.p
          ...
          animate={{ opacity: phase < 3 && phase > 0 ? [0.3, 0.6, 0.3] : 0 }}
